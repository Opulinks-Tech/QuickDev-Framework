...
static T_AppEvtHandlerTbl g_tAppEvtHandlerTbl[] = 
{
    ...
    {APP_EVT_BLE_REQ_BAT_LVL,               APP_EvtHandler_BleReqBatLvl},
...
static void APP_EvtHandler_BleReqBatLvl(uint32_t u32EventId, void *pData, uint32_t u32DataLen)
{
    float fPercent = Bat_AuxAdc_Convert_To_Percentage();

    OPL_LOG_INFO(APP, "Rsp battery level %f", fPercent);

    BAS_Svc_BatteryLevelNotify((uint8_t)fPercent);
}
...
static void APP_EvtHandler_SysTimerTimeout(uint32_t u32EventId, void *pData, uint32_t u32DataLen)
{
    ...
    OPL_LOG_INFO(APP, "Notify battery level %f", fPercent);
