...
void UART_MessageSend(uint8_t *u8aData, uint32_t u32Len)
{
    ...
    OPL_LOG_INFO(UART, "Send to MCU : %s (len %d)", u8aData, u32Len);
    //tracer_drct_printf("[UART] Send to MCU : %s (len %d)\n", u8aData, u32Len);

    for (i = 0; i < u32Len; i ++)
    {
        //tracer_drct_printf("%c", u8aData[i]);
...
void UART_MessageReceive(uint32_t ulData)
{
    ...
    else if(g_u16UartRecvDataCnt != 0 && 0x0A == ulData && 0x0D == g_u8aUartRecvData[g_u16UartRecvDataCnt - 1])
    {
        // g_u8aUartRecvData[g_u16UartRecvDataCnt] = ulData;
        g_u8aUartRecvData[g_u16UartRecvDataCnt - 1] = '\0';
        g_u16UartRecvDataCnt--;
        //tracer_drct_printf("RECV\n");
