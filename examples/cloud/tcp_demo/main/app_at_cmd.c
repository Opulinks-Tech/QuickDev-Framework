...
at_command_t g_taAppAtCmd[] =
{
    ...
    {"at+tcpconnect",      AT_CmdTcpConnectHandler,             "Establish the TCP connection (Input host IP,Port)"},
    {"at+tcpdisconnect",   AT_CmdTcpDisconnectHandler,          "Disconnect the TCP connection"},
    {"at+tcpkalive",       AT_CmdTcpKeepAliveDurationHandler,   "Set the duration of keep alive time"},
    {"at+tcpsend",         AT_CmdTcpSendHandler,                "Post data"},
    {"at+smartsleep",      AT_CmdSmartSleepHandler,             "Enter/Exit smart sleep"},
    // *
...
int AT_CmdTcpConnectHandler(char *buf, int len, int mode)
{
    ...
    switch(mode)
    {
        case AT_CMD_MODE_SET:
        {
            ...
            T_OplErr tEvtRst = Cloud_MsgSend(CLOUD_EVT_TYPE_ESTABLISH, (uint8_t *)&tCloudConnInfo, sizeof(T_CloudConnInfo));

            if(tEvtRst == OPL_ERR_RTOS_QMSG_NOT_INIT || tEvtRst == OPL_ERR_RTOS_TASK_NOT_INIT)
            {
                AT_LOG("cloud task or queue not init\r\n");
            }
...
int AT_CmdTcpKeepAliveDurationHandler(char *buf, int len, int mode)
{
    ...
    switch(mode)
    {
        case AT_CMD_MODE_SET:
        {
            ...
            uint32_t u32KeepAliveDuration = (uint32_t)strtoul(argv[1], NULL, 0);

            AT_LOG("change keep alive time to %d ms\r\n", u32KeepAliveDuration);

            Cloud_KeepAliveDurationSet(u32KeepAliveDuration);
...
int AT_CmdTcpSendHandler(char *buf, int len, int mode)
{
    ...
    switch(mode)
    {
        case AT_CMD_MODE_SET:
        {
            ...
            tCloudPayloadFmt.u32PayloadLen = strlen(argv[1]);
            memcpy(tCloudPayloadFmt.u8aPayloadBuf, argv[1], tCloudPayloadFmt.u32PayloadLen);

            T_OplErr tEvtRst = Cloud_MsgSend(CLOUD_EVT_TYPE_POST, (uint8_t *)&tCloudPayloadFmt, sizeof(T_CloudPayloadFmt));
...
int AT_CmdSmartSleepHandler(char *buf, int len, int mode)
{
    ...
    switch(mode)
    {
        case AT_CMD_MODE_SET:
        {
            ...
            uint8_t u8SmartSleepEn = (uint8_t)atoi(argv[1]);
            uint32_t u32SmartSleepDelay = (uint32_t)strtoul(argv[2], NULL, 0);

            if(u8SmartSleepEn == 0)
            {
                PS_ExitSmartSleep();
            }
            else if(u8SmartSleepEn == 1)
            {
                PS_EnterSmartSleep(u32SmartSleepDelay);
            }
