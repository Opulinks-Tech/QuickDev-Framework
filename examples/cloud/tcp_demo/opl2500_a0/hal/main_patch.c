...
void MreadCmd_Cb( void )
{
    if(0x40 == g_u8RcvData)
    {
        // Response 0x41 for 0x40 command
        uint8_t u8SendData = 0x41;

        g_u8RcvData = 0;
        Hal_I2c_SlaveTrasmit(&u8SendData, 1);
    }
}

void MwriteCmd_Cb(uint8_t u8Data)
{
    if (u8Data == 0x80) // Command to enter deep sleep
    {
        _DeepSleepTrigger();
    }
    else
    if (0x40 == u8Data) // Command to query slave
    {
        g_u8RcvData = u8Data;
    }
}
