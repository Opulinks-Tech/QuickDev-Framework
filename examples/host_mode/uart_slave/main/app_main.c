...
static void APP_EvtHandler_MasterWakeup(uint32_t u32EventId, void *pData, uint32_t u32DataLen)
{
    ...
    tUartData.tUartCmdType = UART_CMD_SLAVE_WAKEUP_SIGN;
    tUartData.u8DataLen = 0;

    // received wakeup signal from master, send wakeup sign
    APP_UartSendingDataProcess(tUartData.tUartCmdType, tUartData.u8aData, tUartData.u8DataLen);
#endif
}
...
T_OplErr APP_UartReceivedDataProcess(uint8_t *pu8UartData)
{
    ...
    {
        case UART_CMD_SMART_SLEEP_ENT_REQ:
        {
            OPL_LOG_INFO(APP, "enter smart sleep");

#if (APP_HOST_MODE_PWR_SAVE_DEMO == 0)
            // received smart sleep enter request, response ack
            tUartData.tUartCmdType = UART_CMD_SMART_SLEEP_ENT_RSP;
            tUartData.u8DataLen = 0;

            APP_SendMessage(APP_EVT_UART_MSG_SEND, (uint8_t *)&tUartData, sizeof(T_UartData));

            // enter smart sleep after 100ms
            PS_EnterSmartSleep(100);
#endif

            break;
        }
        case UART_CMD_DEEP_SLEEP_ENT_REQ:
        {
            OPL_LOG_INFO(APP, "enter deep sleep");

#if (APP_HOST_MODE_PWR_SAVE_DEMO == 1)
            // received deep sleep enter request, response ack
            tUartData.tUartCmdType = UART_CMD_DEEP_SLEEP_ENT_RSP;
            tUartData.u8DataLen = 0;

            // process sending data here directly to avoid the send after sleep
            APP_UartSendingDataProcess(tUartData.tUartCmdType, tUartData.u8aData, tUartData.u8DataLen);

            // enter deep sleep
            PS_EnterDeepSleep(HOST_MODE_WAKEUP_PIN, INT_TYPE_RISING_EDGE);
#endif

            break;
        }
        case UART_CMD_BLE_ADV_START_REQ:
        {
            OPL_LOG_INFO(APP, "start ble adv");
            break;
        }
        case UART_CMD_BLE_ADV_STOP_REQ:
        {
            OPL_LOG_INFO(APP, "stop ble adv");
            break;
        }
        case UART_CMD_WIFI_CONNECT_REQ:
        {
            OPL_LOG_INFO(APP, "connect to WIFI");
            break;
        }
        case UART_CMD_WIFI_DISCONNECT_REQ:
        {
            OPL_LOG_INFO(APP, "disconnect WIFI");
            break;
        }
        case UART_CMD_PING:
        {
            OPL_LOG_INFO(APP, "recv PING");

            // received pong, send slave to sleep
            tUartData.tUartCmdType = UART_CMD_PONG;
            tUartData.u8DataLen = 0;
