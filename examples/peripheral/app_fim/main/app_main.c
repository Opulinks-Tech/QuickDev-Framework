...
uint32_t g_u32SysTmrCnt = 0;
T_MwFimGP21WeightConfig g_tWeightConfig[MW_FIM_GP21_WEIGHT_CONFIG_NUM];
extern const T_MwFimGP21WeightConfig g_tMwFimDefaultGp21WeightConfig;
...
static void APP_EvtHandler_SysTimerTimeout(uint32_t u32EventId, void *pData, uint32_t u32DataLen)
{
    ...
    if(0 == g_u32SysTmrCnt)
    {
        memset(g_tWeightConfig, 0, sizeof(g_tWeightConfig));


        for (i = 0; i < MW_FIM_GP21_WEIGHT_CONFIG_NUM; i++)
        {
            if ( MW_FIM_OK != MwFim_FileWriteDefault(MW_FIM_IDX_GP21_PROJECT_WEIGHT_CONFIG, i) )
            {
                OPL_LOG_ERRO(APP, "Write Default Weight Config fail");
            }

            if ( MW_FIM_OK != MwFim_FileRead( MW_FIM_IDX_GP21_PROJECT_WEIGHT_CONFIG, 
                                              i, 
                                              MW_FIM_GP21_WEIGHT_CONFIG_SIZE, 
                                              (uint8_t*)&(g_tWeightConfig[i])
                                            )
               )

            {
                OPL_LOG_ERRO(APP, "Read Weight Config fail");
                //memcpy( &(g_tWeightConfig[i]), &g_tMwFimDefaultGp21WeightConfig, MW_FIM_GP21_WEIGHT_CONFIG_SIZE);
            }

            OPL_LOG_WARN(APP, "WCID[%d] ID[%s], uint[%s], height[%d], age[%d]", 
                         i,
                         g_tWeightConfig[i].u8aUserID,
                         g_tWeightConfig[i].u8aUintOfWeight,
                         g_tWeightConfig[i].u16Height,
                         g_tWeightConfig[i].u8Age);
        }
    }
    else
    if( g_u32SysTmrCnt >= 1 && g_u32SysTmrCnt <=4)
    {
        switch(g_u32SysTmrCnt)
        {
            case 1:
            {
                strcpy((char *)g_tWeightConfig[g_u32SysTmrCnt-1].u8aUserID, "Dad");
                strcpy((char *)g_tWeightConfig[g_u32SysTmrCnt-1].u8aUintOfWeight, "kg");
                g_tWeightConfig[g_u32SysTmrCnt-1].u16Height = 183;
                g_tWeightConfig[g_u32SysTmrCnt-1].u8Age = 35;
                break;
            }
            case 2:
            {
                strcpy((char *)g_tWeightConfig[g_u32SysTmrCnt-1].u8aUserID, "Mom");
                strcpy((char *)g_tWeightConfig[g_u32SysTmrCnt-1].u8aUintOfWeight, "kg");
                g_tWeightConfig[g_u32SysTmrCnt-1].u16Height = 167;
                g_tWeightConfig[g_u32SysTmrCnt-1].u8Age = 34;
                break;
            }
            case 3:
            {
                strcpy((char *)g_tWeightConfig[g_u32SysTmrCnt-1].u8aUserID, "Daughter");
                strcpy((char *)g_tWeightConfig[g_u32SysTmrCnt-1].u8aUintOfWeight, "lb");
                g_tWeightConfig[g_u32SysTmrCnt-1].u16Height = 163;
                g_tWeightConfig[g_u32SysTmrCnt-1].u8Age = 13;
                break;
            }
            case 4:
            {
                strcpy((char *)g_tWeightConfig[g_u32SysTmrCnt-1].u8aUserID, "Son");
                strcpy((char *)g_tWeightConfig[g_u32SysTmrCnt-1].u8aUintOfWeight, "lb");
                g_tWeightConfig[g_u32SysTmrCnt-1].u16Height = 151;
                g_tWeightConfig[g_u32SysTmrCnt-1].u8Age = 11;
                break;
            }
        ...
        if (MW_FIM_OK != MwFim_FileWrite( MW_FIM_IDX_GP21_PROJECT_WEIGHT_CONFIG, 
                                          g_u32SysTmrCnt-1, 
                                          MW_FIM_GP21_WEIGHT_CONFIG_SIZE, (uint8_t*)&(g_tWeightConfig[g_u32SysTmrCnt-1])
                                        )
            )
        {
            OPL_LOG_ERRO(APP, "Weight config write fail");
        }
        else
        {
            if ( MW_FIM_OK != MwFim_FileRead( MW_FIM_IDX_GP21_PROJECT_WEIGHT_CONFIG, 
                                              g_u32SysTmrCnt-1,
                                              MW_FIM_GP21_WEIGHT_CONFIG_SIZE, 
                                              (uint8_t*)&(g_tWeightConfig[g_u32SysTmrCnt-1])
                                            )
               )
            ...
            OPL_LOG_WARN(APP, "WCID[%d] ID[%s], uint[%s], height[%d], age[%d]", 
                         g_u32SysTmrCnt-1,
                         g_tWeightConfig[g_u32SysTmrCnt-1].u8aUserID,
                         g_tWeightConfig[g_u32SysTmrCnt-1].u8aUintOfWeight,
                         g_tWeightConfig[g_u32SysTmrCnt-1].u16Height,
                         g_tWeightConfig[g_u32SysTmrCnt-1].u8Age);
        }
    }
    else
    {
