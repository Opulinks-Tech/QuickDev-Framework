...
{
    APP_SendMessage(APP_EVT_WIFI_SCAN_DONE, NULL, 0);
}
...
{
    OPL_LOG_INFO(APP, "Wi-Fi scan done, try connect to %s..", APP_DEF_AP_SSID);
...
// scan result
    ...
    {
        OPL_LOG_ERRO(APP, "Connect to %s fail, retry scan after %d ms", APP_DEF_AP_SSID, APP_WIFI_SCAN_RETRY_TIME);
...
{
    OPL_LOG_INFO(APP, "Connect to %s fail, retry scan after %d ms", APP_DEF_AP_SSID, APP_WIFI_SCAN_RETRY_TIME);
...
{
    OPL_LOG_INFO(APP, "Trigger scaning..");

    // trigger scan request
    if(OPL_OK != APP_NmWifiScanReq(APP_NmScanDoneIndCallback))
    {
        OPL_LOG_ERRO(APP, "Trigger scan fail, retry scan after %d ms", APP_WIFI_SCAN_RETRY_TIME);
...
void APP_NetInit(void)
{
    ...
    OPL_LOG_INFO(APP, "Net manager init ok, trigger scan after %d ms", APP_WIFI_SCAN_RETRY_TIME);
