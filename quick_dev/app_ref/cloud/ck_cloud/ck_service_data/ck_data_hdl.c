...
T_CkDataConnCfg g_tCkDataConnCfg = {0};
...
static void _CK_DataHandler_FilterStrongRssiAP(T_WmScanInfo *pstScanInfo ,uint16_t u16apCount);
#endif

static void _CK_DataHandler_SendSignalScanReport(uint16_t apCount, T_WmScanInfo *ap_list);
static int _CK_DataHandler_SendScanReport(void);
static void _CK_DataHandler_SendStatusInfo(uint16_t u16Type);
...
static void _CK_DataHandler_SendSignalScanReport(uint16_t apCount, T_WmScanInfo *ap_list)
{
    ...
    CK_DataSendEncap(CK_DATA_RSP_SCAN_REPORT, data, data_len);
...
static int _CK_DataHandler_SendScanReport(void)
{
    ...
    if (!pstWifiAPList) {
        printf("malloc fail, pstWifiAPList is NULL\r\n");
        ubAppErr = -1;
        goto err;
    }
...
#if (1 == FLITER_STRONG_AP_EN)
    _CK_DataHandler_FilterStrongRssiAP(pstWifiAPList , u16apCount);
#endif
    ...
    for (i = 0; i < u16apCount; ++i)
    {
        ...
        {
            _CK_DataHandler_SendSignalScanReport(1, &pstWifiAPList[i]);
            osDelay(100);
        }
    }
...
{
    printf("BLEWIFI: MSG APP_CTRL_MSG_WIFI_SCAN_DONE %d\r\n", tEvtRst);
    ...
    if(CK_DATA_CONN_TYPE_BSSID == g_tCkDataConnCfg.u8ConnectType)
    {
        // CK_DATA_REQ_SCAN cmd, just do report scan list
        _CK_DataHandler_SendScanReport();
        CK_DataSendResponse(CK_DATA_RSP_SCAN_END, 0);
    }
    else if(CK_DATA_CONN_TYPE_SSID == g_tCkDataConnCfg.u8ConnectType)
    {
        ...
        if(g_u8IsManuallyConnectScanRetry == true)
        {
            ...
            for (i = 0; i < stScanList.num; i++)
            {
                ...
                if ((memcmp(stScanList.ap_record[i].ssid, g_tCkDataConnCfg.u8aSsid, sizeof(g_tCkDataConnCfg.u8aSsid)) == 0) ||
            ...
            if(true == u8IsMatched)
            {
                ...
                stConnConfig.u8SsidLen = g_tCkDataConnCfg.u8SsidLen;
                stConnConfig.u8PwdLen = g_tCkDataConnCfg.u8PwdLen;
                memcpy(stConnConfig.u8aSsid, stScanList.ap_record[i].ssid, stConnConfig.u8SsidLen);
                memcpy(stConnConfig.u8aPwd, g_tCkDataConnCfg.u8aPwd, g_tCkDataConnCfg.u8PwdLen);
            ...
            {
                CK_DataSendResponse(CK_DATA_RSP_CONNECT, CK_DATA_WIFI_AP_NOT_FOUND);
            }
        ...
        else
        {
            ...
            {
                printf("[CK Data] manually conn - scan retry\r\n");
...
{
    printf("BLEWIFI: MSG APP_CTRL_MSG_WIFI_CONNECTION %d\r\n", tEvtRst);

    if(OPL_OK == tEvtRst)
    {
        printf("[CK Data] Replay connect success\r\n");

        // send connect success
        CK_DataSendResponse(CK_DATA_RSP_CONNECT, CK_DATA_WIFI_CONNECTED_DONE);
        
        // send ip message
        _CK_DataHandler_SendStatusInfo(CK_DATA_IND_IP_STATUS_NOTIFY);
    }
    else
    {
        printf("[CK Data] Replay connect fail\r\n");
        // send connect fail
...
{
    CK_DataSendResponse(CK_DATA_RSP_DISCONNECT, tEvtRst);
}
