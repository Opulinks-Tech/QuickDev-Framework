...
IoT_Error_t aws_iot_thread_mutex_init(IoT_Mutex_t *pMutex) {
    ...
    osSemaphoreDef_t tSemaphoreDef_aws;
    tSemaphoreDef_aws.dummy = 0;                            // reserved, it is no used
    pMutex->lock= osSemaphoreCreate(&tSemaphoreDef_aws, 1);
    if (pMutex->lock == NULL)
    {
        printf("To create the semaphore for AWS_state_change is fail.\n");
...
//int rc = pthread_mutex_lock(&(pMutex->lock));
    int rc = osSemaphoreWait(pMutex->lock, osWaitForever);
...
IoT_Error_t aws_iot_thread_mutex_unlock(IoT_Mutex_t *pMutex) {
    ...
    if(0 != osSemaphoreRelease(pMutex->lock)) {
...
IoT_Error_t aws_iot_thread_mutex_destroy(IoT_Mutex_t *pMutex) {
	//if(0 != pthread_mutex_destroy(&(pMutex->lock))) {  //osMutexDelete
	if(0 != osSemaphoreDelete(pMutex->lock)) {  //osMutexDelete
